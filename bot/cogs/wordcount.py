...
class WordCounter(commands.Cog, name="Word Count"):
    ...
    async def on_message(self, message):
        ...
        for word in self.word_list:
            if word.word in message.content.lower():
                word.count += 1
                await message.channel.send(f"Word `{word.word}` has been said {word.count} times!")
                self.client.db_session.commit()
    
    @commands.command(name="addword", description="Add a new word to track the count of")
    ...
    @commands.command(name="removeword", description="Remove a word from being tracked")
