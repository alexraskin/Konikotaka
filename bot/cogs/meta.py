...
class Meta(commands.Cog, name="Meta"):
    def __init__(self, client: commands.Bot) -> None:
        ...
        self.rand_number = f"{str(self.random_number)[:-4]}-{str(self.random_number)[-4:]}"
        ...
        self. font = ImageFont.truetype(f"{self.file_path}/files/runescape_uf.ttf", size=34)
        self.user_font = ImageFont.truetype(f"{self.file_path}/files/runescape_uf.ttf", size=45)
    ...
    async def on_member_join(self, member: Union[Member, User]) -> None:
        ...
        try:
          ...
          channel = await self.client.fetch_channel(1145087802141315093)
          await channel.send(
