...
class SnadFun(commands.Cog, name="Fun"):
  ...
  async def snad(self, ctx: commands.Context):
    ...
    await ctx.send(f"Snad has been caught {snad_count} times! <:LULW:1146225785515016342>")
  ...
  async def add_snad(self, ctx: commands.Context):
    try:
      ...
      snad_count = SnadCounter(
        count=len(count),
        caught=datetime.now()
      )
      self.client.db_session.add(snad_count)
      self.client.db_session.commit()
      await ctx.send(f"Snad has been caught {len(count) + 1} times!")
